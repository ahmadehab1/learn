...
var performancesCroppersArr = [{ ord: 1, width: 810, height: 387 }, { ord: 2, width: 100, height: 100 }, { ord: 3, width: 400, height: 400 }, { ord: 4, width: 250, height: 130 }, { ord: 5, width: 1274, height: 928 }];
var eventsCroppersArr = [{ ord: 1, width: 810, height: 387 }, { ord: 2, width: 100, height: 100 }, { ord: 3, width: 400, height: 400 }, { ord: 4, width: 250, height: 130 }];
var performersCroppersArr = [{ ord: 1, width: 1200, height: 313 }, { ord: 2, width: 400, height: 400 }];
var venuesCroppersArr = [{ ord: 1, width: 250, height: 130 }, { ord: 2, width: 400, height: 313 }];
//HS:Check the current controller
switch (controller) {
    case 'Performances':
        initializeCropper('perfc', performancesCroppersArr);
        initialize();
        break;
    case 'Events':
        initializeCropper('ev', eventsCroppersArr);
        break;
    case 'Performers':
        initializeCropper('perfr', performersCroppersArr);
        break;
    case 'Venues':
        initializeCropper('ven', venuesCroppersArr);
}
//HS:Initialize all zoomable croppers on the page
...
function initialize() {
    var imageName;
    $("#perfc-img6-dir-upload").addClass('disabled');
    if (editing && $("#perfc-img6-name").val() !== "") {
        $("#perfc-img6").attr("src", $("#perfc-img6-name").val());
    }
    else {
        $("#perfc-img6-img-overlay-text").show();
    }
    $(".upload-btn6").on("click", function () {
        $(this).attr("disabled", "disabled");
        $(document).on("focus", "input", function () {
            $(".upload-btn6").removeAttr("disabled");
        })
    })
    $('#perfc-img6-upload').on('change', function (event) {
        ...
        if (files.length > 0) {
            var file = files[0],
                img = new Image(),
                itsContainer = $("#perfc-img6-img-container"),
                itsContainerRatio = itsContainer.width() / itsContainer.height();
            ...
            $(img).on('load', function () {
                ...
                if (ratio > itsContainerRatio) {
                    $("#perfc-img6").css("width", "100%");
                    $("#perfc-img6").css("height", "auto");
                }
                else {
                    $("#perfc-img6").css("height", "100%");
                    $("#perfc-img6").css("width", "auto");
                }
            });
            img.src = '';
            img.src = _URL.createObjectURL(file);
            $("#perfc-img6-dir-upload").removeClass('disabled');
            $("#perfc-img6-img-overlay-text").hide();
            imageName = files[0].name;
            readFile(event, "perfc", 6);
        }
    });
    $("#perfc-img6-dir-upload").on("click", function () {
        upload($("#perfc-img6").attr("src"), 'perfc', 6, imageName);
    })
}
//HS:Loop function
...
function upload(base64, page, num, fileName) {
    ...
    // var buf = new Buffer(src.replace(/^data:image\/\w+;base64,/, ""), 'base64')
